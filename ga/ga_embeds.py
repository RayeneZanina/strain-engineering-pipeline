...
class GeneticAlgorithm_embeds:
    def __init__(self,model, gnn , base_graph, target_id, biomass_id, population_size=100, mutation_rate=0.01, crossover_rate=0.5, elitism_rate=0.1, max_generations=200):
    ...
    def get_embedding(self, individual):
        if tuple(individual) in self.embedding_cache:
            return self.embedding_cache[tuple(individual)]
        _, emb = self.gnn(self.base_graph, individual.astype(float), self.biomass_rxn.id, self.target_rxn.id)
        self.embedding_cache[tuple(individual)] = emb
        return emb
    ...
    def evaluate_population(self):
        rewards = np.array([self.evaluate_fitness(individual) for individual in self.population])
        embeddings = np.array([self.get_embedding(individual).detach().numpy() for individual in self.population])
        novelty = []
        for i, emb_i in enumerate(embeddings):
            distances = [np.linalg.norm(emb_i - emb_j) for j, emb_j in enumerate(embeddings) if i != j]
            novelty.append(np.mean(distances))
        fitness_scores = rewards + 0.1 * np.array(novelty)
