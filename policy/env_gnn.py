...
class MetabolicEnv_gnn(gym.Env):
    ...
    def step(self, action):
        ...
        with torch.no_grad():
            pred, embedding = self.gnn(self.graph, knockout_mask, self.biomass_rxn.id, self.target_rxn.id)
            pred, embedding = pred.detach().numpy(), embedding.detach().numpy()
        target_flux, biomass_flux = np.expm1(pred[0]), np.expm1(pred[1])
